...
DST_PORT = 993
DST_USER = "ectr@normassistance.com.tr"
DST_PASS = "Ectr2426"
...
def migrate_folder(folder):
    print(f"\n==========================================", flush=True)
    print(f"Starting Migration for folder: {folder}", flush=True)
    print(f"==========================================", flush=True)
    ...
    # Check how many are already on destination to resume
    typ, dst_data = dst.search(None, "ALL")
    already_copied = len(dst_data[0].split()) if typ == "OK" and dst_data[0] else 0
    print(f"Destination currently has: {already_copied} messages in {folder}", flush=True)
    ...
    if already_copied >= total_src:
        print(f"Folder {folder} already fully migrated!", flush=True)
        try:
            src.logout()
            dst.logout()
        except Exception:
    ...
    for idx, num in enumerate(pending_ids):
        # Refresh connection every 250 messages
        ...
        # Retry loop for copying individual message
            ...
            try:
                ...
                if res == "OK":
                    ...
                    if current_total % 100 == 0 or current_total == total_src:
                        ...
                        print(f"[{folder}] Migrated {current_total}/{total_src} messages ({current_total*100//total_src}%) - {speed:.1f} msg/sec", flush=True)
                    break
                else:
    ...
    try:
        ...
        pass
    print(f"Folder {folder} completed!", flush=True)

def main():
    print("=== Continuous Mail Migration Engine Started ===", flush=True)
    migrate_folder("INBOX")
    migrate_folder("Sent")
    print("=== ALL FOLDERS FULLY MIGRATED! ===", flush=True)
